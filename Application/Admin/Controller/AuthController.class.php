...
//******************权限***********************
    ...
    public function index(){
        $data=D('AuthRule')->getTreeData('tree','pid','title');
        //dump($data);exit;
        $assign=array(
            'data'=>$data
        );
...
//******************用户-用户组*******************
    ...
    public function check_user(){
        ...
        if(empty($username)){
            ...
            $user_data=M('UcenterMember')->where(array('username'=>$username))->select();
        }
        $assign=array(
    ...
    public function add_user_to_group(){
        ...
        $this->success('操作成功',U('check_user',array('group_id'=>$data['group_id'],'username'=>$data['username'])));
    }
    ...
    public function delete_user_from_group(){
        ...
        $this->success('操作成功',U('admin_user_list'));
    }
    ...
    public function add_admin(){
        $ucenter_member = D('UcenterMember');
        if(IS_POST){
            $data=I('post.');
            $result=$ucenter_member->addData($data);
            if($result){
                ...
                // 操作成功
                $this->success('添加成功',U('admin_user_list'));
            }else{
                $error_word=$ucenter_member->getError();
                // 操作失败
    ...
    public function edit_admin(){
        ...
        if(IS_POST){
            ...
            foreach ($data['group_ids'] as $k => $v) {
                ...
                $result = D('AuthGroupAccess')->addData($group);
            }
            if($result){
                // 操作成功
                $this->success('编辑成功',U('admin_user_list'));
            }else{
                $error_word=$ucenter_member->getError();
                if (empty($error_word)) {
                    $this->success('编辑成功',U('admin_user_list'));
            ...
            // 获取用户数据
            $user_data=$ucenter_member->find($id);
            // 获取已加入用户组
