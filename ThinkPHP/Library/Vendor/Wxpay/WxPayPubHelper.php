...
class MicroPay extends Common_util_pub{
    ...
    public function GetResult(){
        $xml = $this->createXml();
        return $this->postXmlCurl($xml, $this->url);
    }

    /*
     * $result wx返回结果, $orderid 订单id
     * */
    //处理支付结果
    public function Treat($result, $orderid){
        ...
        if($result['result_code'] == "FAIL"){
            switch ($result['err_code']){
                case "USERPAYING":
                    ...
                    //noncestr已填,商户无需重复填写
                    //sign已填,商户无需重复填写
                    $orderQuery->setParameter("out_trade_no", $orderid);//商户订单号
                    //非必填参数，商户可根据实际情况选填
                    $orderQuery->setParameter("sub_mch_id", WxPayConf_pub::SUBMCHID);//子商户号
                    //$orderQuery->setParameter("transaction_id","XXXX");//微信订单号

                    //获取订单查询结果
                    return $this->arrayToXml($orderQuery->getResult());
                    break;
                default:
