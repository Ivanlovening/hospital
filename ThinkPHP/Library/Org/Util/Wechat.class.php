...
class Wechat
{
    ...
    protected function log($log_content){
        if(isset($_SERVER['HTTP_APPNAME'])){   //SAE
            ...
            $max_size = 10000000;
            $log_filename = "/var/www/html/yft/log/log.xml";
            if(file_exists($log_filename) and (abs(filesize($log_filename)) > $max_size)){unlink($log_filename);}
            file_put_contents($log_filename, "-----------------开始 begin----------------\n".date('H:i:s')."\n".$log_content."\n-----------------结束 end----------------\n\n\n", FILE_APPEND);
        }
    }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    //保存access_token成getaccesstoken.json文件
    public function getAccessToken2() {
        If (!$this->access_token && !$this->checkAuth()) return false;
        $data = json_decode(file_get_contents("getaccesstoken.json"));
        if ($data->access_token != $this->access_token) {
            $access_token = $this->access_token;
            if ($access_token) {
                $data->access_token = $access_token;
                $fp = fopen("getaccesstoken.json", "w");
